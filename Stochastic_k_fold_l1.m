function lambda=Stochastic_k_fold_l1(train_X,train_y,k)
...
[m,n] = size(train_X) ;
    ...
    for i=[1:k]
        ...
        for j=[1:length(lambda_set)]
            w=stochastic_smoothed_regression1(train_X,train_y,lambda_set(j));
    ...
    title(['Lasso Regularisation K fold cross validation, k=',num2str(k), ', N=',num2str(m)])
    dim = [0.2 0.5 0.3 0.3];
    str = {['Optimal \lambda=',num2str(lambda)],['Average MAE=',num2str(avg_mae)]};
